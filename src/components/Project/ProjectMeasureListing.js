...
export class ProjectMeasureListing extends React.Component {
  static propTypes = {
  ...
  filterLibrary = (filteredProjects, value, filter) => {
    ...
    return Object.keys(appliedFilterValues).reduce((projects, key) => {
      ...
      if (key === 'type') {
        ...
        return projects.filter(project => {
          let types = project.applicable_building_types || []
          if (isEmpty(types)) return true
          return types.indexOf(appliedFilterValues[key]) !== -1
        })
      }
          ...
          <div className={projectStyles.projectListingTools}>
            <div className={projectStyles.projectListingFilter}>
              ...
              {Object.keys(filterValues).map((value, index) => {
                ...
                if (value === 'tech' || value === 'app') return null
                if (value == 'category') {
                  return (
                    <div key={index} className={projectStyles.categoryWrapper}>
                      <Categorization
                        category={filterValues['category']}
...
export default ProjectMeasureListing
