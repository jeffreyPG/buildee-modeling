...
class FilterContainer extends Component {
  static propTypes = {
    ...
    buildingGroups: PropTypes.array,
    toggleEditGroup: PropTypes.func
  }
  ...
  removeFilter = (event, item) => {
    ...
    if (item.select !== 'multiSelect') {
      let index = filters.findIndex(filter => filter.value === item.value)
      handleFilterChange([
      ...
      let newFilters =
        filters.filter(filter => filter.value !== item.value) || []
  ...
  handleFilterOptionSelect = (options, selectedAll) => {
    ...
    this.setState({ selectedFilterItem: null })
  }
  ...
  renderFilterComponent = (item, handleFilter) => {
    const isNotActive = this.checkNotActive(item)
    let itemList = isNotActive ? this.getItemList(item) : this.props.itemList
    switch (item.select) {
      case 'multiSelect':
        return (
          <MultiSelect
  ...
  renderFilterText = item => {
    ...
    switch (item.select) {
      ...
      case 'costRange':
        text = `${formatStringUpperCase(item.label)} : ${
          options.option
        } ${formatUnit(item.unit, options.cost)}`
        break
      default: {
  ...
  checkNotActive = item => {
    ...
    if (!item || item === -1) return isNotActive
    if (
      item.tab == this.props.activeTab ||
      item.value.includes('organization') ||
      ((this.props.activeTab === 'project' ||
        this.props.activeTab === 'projectPackage') &&
        item.value === 'building.buildingname') ||
      item.tab === 'all'
    )
      isNotActive = false
    return isNotActive
        ...
        <div className={classNames(styles.panelShowFilter)}>
          ...
          >
            ...
            {filters.map((item, index) => {
              ...
              if (this.isAlreadyShownMultiSelect(item, index)) return null
              let isNotActive = this.checkNotActive(item)
              return (
                <div
                  className={classNames(styles.showFilterSelect)}
                  key={`filter-${index}`}
                  onClick={() => {
                    this.handleEditFilter(item, index)
                  }}
                >
                  <div
                    className={
                      isNotActive ? styles.showFilterSelectDisable : ''
                    }
                  >
                    {text}&nbsp;
                  </div>
