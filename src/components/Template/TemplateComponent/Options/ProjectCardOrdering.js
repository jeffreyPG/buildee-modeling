...
import templateStyle from './TemplateOptions.scss'
...
export class ProjectOrdering extends Component {
  ...
  setCardIcon = (images = [], option) => {
    if (!images) return
    let body = JSON.parse(JSON.stringify(this.props.body))
    if (!body[this.props.index].projectConfig)
      body[this.props.index].projectConfig = {}
    let card = body[this.props.index]
    if (!card.projectConfig.filter) card.projectConfig.filter = {}
    if (!card.projectConfig.filter.category)
      card.projectConfig.filter.category = []
    let categories = card.projectConfig.filter.category || []
    let selectedCategory = _.find(categories, { name: option })
    selectedCategory['image'] = images && images.length > 0 ? images[0] : []
    this.props.bodyTemplate(body)
    this.props.templateUpdateError()
  }

  handleChangeColor = (color, target, option) => {
    let body = JSON.parse(JSON.stringify(this.props.body))
    if (!body[this.props.index].projectConfig)
      body[this.props.index].projectConfig = {}
    let card = body[this.props.index]
                  ...
                  <div key={index}>
                    ...
                    {projectConfig.filter[filter]
                      ...
                      .map((option, index) => {
                        ...
                        let images = (option && option.image) || undefined
                        ...
                        return (
                          <div key={index} className={styles.orderDetails}>
                            ...
                            {projectConfig && filter === 'category' && (
                              <div
                                className={styles.projectCardColor}
                                key={option.name}
                              >
                                  ...
                                  <CardIconUpload
                                    ...
                                    onFieldUpdate={images => {
                                      if (Object.keys(images).length > 0)
                                        ...
                                        this.setCardIcon([], option.name)
                                      }
                                    }}
                                  />
                                </div>
                                {colorOptions.map((colorOption, index) => {
                                  let color = option[colorOption.value] || ''

                                  return (
                                    <div
                                      className={classNames(
                                        templateStyle['editor-divider__color'],
                                        styles.projectCardColorItem
                                      )}
                                      key={index}
                                    >
