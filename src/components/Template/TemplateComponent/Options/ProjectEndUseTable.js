...
export class ProjectEndUseTable extends Component {
  static propTypes = {
  ...
  handleYearRange = event => {
    ...
    let body = JSON.parse(JSON.stringify(this.props.body))
    let widget = body[index]
    let yearIndex = widget.metaData.yearRange
    if (id === 'Range') {
  ...
  selectHeading = (event, field, heading, target) => {
  ...
  saveHeading = (event, value = '') => {
    let { selectedField, selectedTarget } = this.state
    event.preventDefault()
    if (selectedField) {
      let body = JSON.parse(JSON.stringify(this.props.body))
      let card = body[this.props.index]
      ...
      const index = findIndex(customLabels, {
        field: `${selectedTarget}.${selectedField.value}`
      })
  ...
  checkboxFieldsFormat = (array, type, target) => {
    const { index, body } = this.props
    const card = body[index]
    const customLabels =
      (card &&
        ...
        card.projectConfig.data.customLabels) ||
      []
    const { selectedField, customHeading, selectedTarget } = this.state
    return array.map((item, i) => {
      let checkboxChecked = false
      if (
        type === 'data' &&
              ...
              <div className={tableStyles.fieldNameContainer}>
                ...
                {checkboxChecked && (
                  <i
                    ...
                    onClick={e => this.selectHeading(e, item, name, target)}
                  >
                    edit
                  </i>
                )}
  ...
  render() {
    ...
    if (!projectConfig.data) projectConfig.data = [fields]
    ...
    const dataFields = [
      ...
      { name: 'Total Energy Reduction ( % )', value: 'total_energy_reduction' }
    ]
    const proposedDataFields = [
      { name: 'Electric Savings', value: 'electric_use' },
      { name: 'Natural Gas Savings', value: 'natural_usage' },
      {
        name: 'Proposed Total Energy Usage (kBtu)',
        value: 'total_energy'
      },
      { name: 'Total Energy Reduction ( % )', value: 'total_energy_reduction' },
    ...
    const orderByFields = proposedDataFields.filter(object => {
    ...
    return (
      <div>
        ...
        <div className={styles.data}>
          {projectConfig &&
            ...
            projectConfig.data.fields.map((item, index) => {
              ...
              let field = dataFields.find(({ name, value }) => {
                return value === item
              })
        ...
        <div className={styles.checkboxContainer}>
          {this.checkboxFieldsFormat(dataFields, 'data', 'data')}
          <div
        ...
        <div className={styles.data}>
          {projectConfig &&
            ...
            projectConfig.data.fields.map((item, index) => {
              ...
              let customLabels =
                (projectConfig.data && projectConfig.data.customLabels) || []
              let field = proposedDataFields.find(({ name, value }) => {
                return value === item
              })
        ...
        <div className={styles.checkboxContainer}>
          {this.checkboxFieldsFormat(proposedDataFields, 'data', 'proposed')}
          <div
            className={styles.dataClear}
            onClick={() => this.clearDataFields()}
          >
...
export default withConnect(ProjectEndUseTable)
